...
class App(tk.Frame):
    ...
    def findPaths(self):
        ...
        for lstPath in self.lstSuccessPaths:
            print(lstPath)
        print()
    
    def findPath(self, step, lstPath, preRoad, i, j):
            ...
            if (self.cells[i][j].road == (leftShift2 + preRoad) % 15) \
                ...
                if newPreRoad == 1:
                    if self.cells[i-1][j].end and self.cells[i-1][j].road == 4:
                        ...
                        #print()
                        return
                    self.findPath(step+1, newLstPath, newPreRoad, i-1, j)
                elif newPreRoad == 2:
                    if self.cells[i][j+1].end and self.cells[i][j+1].road == 8:
                        self.lstSuccessPaths.append(newLstPath)
                        #print()
                        return
                    self.findPath(step+1, newLstPath, newPreRoad, i, j+1)
                elif newPreRoad == 4:
        ...
        if preRoad != 4 and i - 1 >= 0: #如果向上可走
            ...
            if self.cells[i-1][j].canMove:
                self.findPath(step+1, newLstPath, 1, i-1, j)
            else:
                if not self.cells[i-1][j].start and not self.cells[i-1][j].end:
                    tPreRoad = 1
                    leftShift2 = tPreRoad * 4 % 15
                    if (self.cells[i-1][j].road == (leftShift2 + preRoad) % 15) \
                        or (self.cells[i-1][j].road == (leftShift2 + preRoad * 2 % 15) % 15) \
                        or (self.cells[i-1][j].road == (leftShift2 + preRoad * 8 % 15) % 15):
                        self.findPath(step+1, newLstPath, 1, i-1, j)
        if preRoad != 8 and j + 1 < self.cols: # 如果向右可走
            ...
            if self.cells[i][j+1].canMove:
                self.findPath(step+1, newLstPath, 2, i, j+1)
            else:
                if not self.cells[i][j+1].start and not self.cells[i][j+1].end:
                    tPreRoad = 2
                    leftShift2 = tPreRoad * 4 % 15
                    if (self.cells[i][j+1].road == (leftShift2 + preRoad) % 15) \
                        or (self.cells[i][j+1].road == (leftShift2 + preRoad * 2 % 15) % 15) \
                        or (self.cells[i][j+1].road == (leftShift2 + preRoad * 8 % 15) % 15):
                        self.findPath(step+1, newLstPath, 2, i, j+1)
        if preRoad != 1 and i + 1 < self.rows: # 如果向下可走
            ...
            if self.cells[i+1][j].end and self.cells[i+1][j].road == 1:
                ...
                return
            if self.cells[i+1][j].canMove:
                self.findPath(step+1, newLstPath, 4, i+1, j)
            else:
                if not self.cells[i+1][j].start and not self.cells[i+1][j].end:
                    tPreRoad = 4
                    leftShift2 = tPreRoad * 4 % 15
        ...
        if preRoad != 2 and j - 1 >=0: # 如果向左可走
            ...
            if self.cells[i][j-1].end and self.cells[i][j-1].road == 2:
                ...
                return
            if self.cells[i][j-1].canMove:
                self.findPath(step+1, newLstPath, 8, i, j-1)
            else:
                if not self.cells[i][j-1].start and not self.cells[i][j-1].end:
                    tPreRoad = 8
                    leftShift2 = tPreRoad * 4 % 15
                    if (self.cells[i][j-1].road == (leftShift2 + preRoad) % 15) \
                        ...
                        or (self.cells[i][j-1].road == (leftShift2 + preRoad * 8 % 15) % 15):
                        self.findPath(step+1, newLstPath, 8, i, j-1)
        #lstPath = copy.deepcopy(newLstPath)
