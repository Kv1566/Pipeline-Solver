...
class App(tk.Frame):
    ...
    def initUI(self):
        ...
        #
        # 初始化視窗
        self.parent.title("Pepeline Solver")
    ...
    def cells_setting(self):
        ...
        #
        self.btnSolve = tk.Button(self.pwInner, text='Solve', bg='yellow', width=int(w*0.09), command=self.solve)
        self.pwInner.add(self.btnSolve)
        #
        self.btnFindPaths = tk.Button(self.pwInner, text='findPath', bg='#00cc00', command=self.findPaths)
        self.pwInner.add(self.btnFindPaths)
        #
        self.pwCells.add(self.pwInner)
    ...
    def solve(self):
        ...
        self.isSuccess = False
        self.times = 0
        self.move(0, self.cells, self.lstGroup, self.lstMoveSteps)
        #print(self.lstGroup)
        #print(df)
        #for thread in self.threadings:
        #    thread.join()
    
    def move(self, steps, cells, lstGroup, lstMoveSteps):
        for i in range(len(lstGroup)):
            if steps >= self.maxSteps:
                return
            #if len(self.isSuccess) > 0:
                #if self.isSuccess[self.threadIdx]:
            if self.isSuccess:
                #print('1------')
                ...
                return
            if len(lstMoveSteps) > 0:
                if (lstGroup[i] == lstMoveSteps[-1]) and (len(lstGroup[i][2]) < 3):
                    continue
                elif (lstGroup[i][2] == lstMoveSteps[-1][2]) \
                    and (lstGroup[i][0] == lstMoveSteps[-1][0]) \
                    and (lstGroup[i][1] != lstMoveSteps[-1][1]):
                    continue
            newCells = copy.deepcopy(cells)
            newLstGroup = copy.deepcopy(lstGroup)
            newLstMoveSteps = copy.deepcopy(lstMoveSteps)
            #time.sleep(1)
            #print(self.times, i)
            #for x in range(self.rows):
            #    for y in range(self.cols):
            ...
            #             cells[x][y].moveNoMove, cells[x][y].rotate, cells[x][y].move2_canMove)
            #print(lstGroup)
            (newCells, newLstMoveSteps, needRefreshOfLstGroup) = self.thisMove(newLstGroup[i], newLstMoveSteps, newCells, steps)
            #time.sleep(1)
            #time.sleep(10)
            #preLenGroup = len(newLstGroup)
            if needRefreshOfLstGroup:
                newLstGroup = self.getMovableGroup(newCells)                    
                #time.sleep(1)
            ...
            if steps > self.maxSteps - 3:
                #print(len(self.isSuccess))
                #print(self.threadIdx)
                self.isSuccess = self.check(newCells)
                #self.isSuccess[self.threadIdx-1] = self.check(newCells)
                #time.sleep(1)
                #if self.isSuccess[self.threadIdx-1]:
                if self.isSuccess:
                    #print('2-----')
                    #for x in range(self.rows):
                    #    for y in range(self.cols):
                    ...
                    for i in range(len(newLstMoveSteps)):
                        if newLstMoveSteps[i][1]:
                            (x1, y1) = newLstMoveSteps[i][2][0]
                            if newLstMoveSteps[i][0] == True:
                                s += 'row '+str(x1)+' right -->\n'
                            else:
                                s += 'col '+str(y1)+' down -->\n'
                        else:
                            (x1, y1) = newLstMoveSteps[i][2][len(newLstMoveSteps[i][2])-1]
                            if newLstMoveSteps[i][0] == True:
                                s += 'row '+str(x1)+' left -->\n'
                            else:
                                s += 'col '+str(y1)+' up -->\n'
                    ...
                    #self.lstMoveSteps.clear()
                    break
            #self.threadIdx += 1
            #self.isSuccess.append(False)
            #if len(newLstGroup)**(self.maxSteps - steps) <= 500000:
            self.move(steps+1, newCells, newLstGroup, newLstMoveSteps)
            #else:
            #    self.threadings.append(threading.Thread(target=self.move, args=[steps+1, newCells, newLstGroup, newLstMoveSteps]))
            #    self.threadings[self.threadIdx].start()
            ...
            if len(lstGroup) != len(newLstGroup):
                i = -1
    
    def thisMove(self, lstGroup, lstMoveSteps, cells, steps):
        self.times+=1
            ...
            for i in range(self.rows):
                for j in range(self.cols):
                    if cells[i][j].move2_canMove:
                        ...
                        needRefreshOfLstGroup = True
        lstMoveSteps.append(lstGroup)
        #print(self.times, steps, self.lstMoveStepsString)
        #print()
        return (cells, lstMoveSteps, needRefreshOfLstGroup)
    ...
    def check(self, cells):
        ...
        if preRoad == 1:
            ...
            j -= 1
        while(True):
            leftShift2 = preRoad * 4 % 15
            if cells[i][j].end == True:
                if cells[i][j].road == leftShift2:
                    isFinished = True
                    print('Successed')
                    break
            if (cells[i][j].road == (leftShift2 + preRoad) % 15) \
                or (cells[i][j].road == (leftShift2 + preRoad * 2 % 15) % 15) \
                or (cells[i][j].road == (leftShift2 + preRoad * 8 % 15) % 15):
                preRoad = cells[i][j].road - leftShift2
            else:
                break
            if preRoad == 1:
                i -= 1
    ...
    def findPaths(self):
        self.timeStart = time.time()
        self.getCells()
        self.lstSuccessPaths = []
        self.lstSuccessPathsType = []
        (i, j) = self.start
        preRoad = self.cells[i][j].road
        if preRoad == 1:
            i -= 1
        elif preRoad == 2:
            j += 1
            ...
            j -= 1
        self.findPath(1, [], [], preRoad, i, j)
        newLstSuccessPaths = []
        newLstSuccessPathsType = []
        for (lstPath, lstPathType) in zip(self.lstSuccessPaths, self.lstSuccessPathsType):
            for (cell, cellType) in zip(lstPath, lstPathType):
                (i, j) = cell
                if not self.cells[i][j].canMove:
                    n = lstPath.index((i, j))
                    lstPath.pop(n)
                    lstPathType.pop(n)
            newLstSuccessPaths.append(lstPath)
            newLstSuccessPathsType.append(lstPathType)
        self.lstSuccessPaths = newLstSuccessPaths
        self.lstSuccessPathsType = newLstSuccessPathsType
        for (lstPath, lstPathType) in zip(self.lstSuccessPaths, self.lstSuccessPathsType):
            tmpLstPath = copy.deepcopy(lstPath)
            tmpLstPathType = copy.deepcopy(lstPathType)
            for i in range(self.rows):
                for j in range(self.cols):
                    if self.cells[i][j].canMove:
                        n = -1
                        if self.cells[i][j].road in tmpLstPathType:
                            n = tmpLstPathType.index(self.cells[i][j].road)
                        if n >= 0:
                            tmpLstPath.pop(n)
                            tmpLstPathType.pop(n)
                            if len(tmpLstPath) == 0:
                                sPath = ''
                                #sPathType = ''
                                for (cell, cellType) in zip(lstPath, lstPathType):
                                    (i, j) = cell
                                    if self.cells[i][j].canMove:
                                        sPath += f'[({i}, {j}),{cellType}], '
                                        #sPathType += f'{cellType},'
                                print(sPath)
                                #print(sPathType)
        print()
    
    def findPath(self, step, lstPath, lstPathType, preRoad, i, j):
        newLstPath = copy.deepcopy(lstPath)
        newLstPathType = copy.deepcopy(lstPathType)
        #print(f'step:{step}, preRoad:{preRoad}, ({i}, {j}), lstPath: {newLstPath}')
        if (i, j) in newLstPath:
            return
        else:
            #if self.cells[i][j].canMove:
            newLstPath.append((i, j))
            newLstPathType.append(preRoad * 4 % 15)
        if not self.cells[i][j].canMove and not self.cells[i][j].start and not self.cells[i][j].end:
            #print(f'step:{step}, preRoad:{preRoad}, ({i}, {j}), lstPath: {newLstPath}')
            leftShift2 = preRoad * 4 % 15
            if (self.cells[i][j].road == (leftShift2 + preRoad) % 15) \
                or (self.cells[i][j].road == (leftShift2 + preRoad * 2 % 15) % 15) \
                or (self.cells[i][j].road == (leftShift2 + preRoad * 8 % 15) % 15):
                newPreRoad = self.cells[i][j].road - leftShift2
                if newPreRoad == 1:
                    if self.cells[i-1][j].end and self.cells[i-1][j].road == 4:
                        newLstPathType[-1] += 1
                        self.lstSuccessPaths.append(newLstPath)
                        self.lstSuccessPathsType.append(newLstPathType)
                        return
                    newLstPathType[-1] += newPreRoad
                    self.findPath(step+1, newLstPath, newLstPathType, newPreRoad, i-1, j)
                    newLstPathType[-1] -= newPreRoad
                elif newPreRoad == 2:
                    if self.cells[i][j+1].end and self.cells[i][j+1].road == 8:
                        newLstPathType[-1] += 2
                        self.lstSuccessPaths.append(newLstPath)
                        self.lstSuccessPathsType.append(newLstPathType)
                        return
                    newLstPathType[-1] += newPreRoad
                    self.findPath(step+1, newLstPath, newLstPathType, newPreRoad, i, j+1)
                    newLstPathType[-1] -= newPreRoad
                elif newPreRoad == 4:
                    if self.cells[i+1][j].end and self.cells[i+1][j].road == 1:
                        newLstPathType[-1] += 4
                        self.lstSuccessPaths.append(newLstPath)
                        self.lstSuccessPathsType.append(newLstPathType)
                        return
                    ...
                    if self.cells[i][j-1].end and self.cells[i][j-1].road == 2:
                        newLstPathType[-1] += 8
                        self.lstSuccessPaths.append(newLstPath)
                        self.lstSuccessPathsType.append(newLstPathType)
                        return
                    ...
                    newLstPathType[-1] -= newPreRoad
            return            
        if preRoad != 4 and i - 1 >= 0: #如果向上可走
            #print(f'step:{step}, preRoad:{preRoad}, ({i}, {j})')
            if self.cells[i-1][j].end and self.cells[i-1][j].road == 4:
                newLstPathType[-1] += 1
                self.lstSuccessPaths.append(newLstPath)
                self.lstSuccessPathsType.append(newLstPathType)
                return
            if self.cells[i-1][j].canMove:
                newLstPathType[-1] += 1
                self.findPath(step+1, newLstPath, newLstPathType, 1, i-1, j)
                newLstPathType[-1] -= 1
            else:
                if not self.cells[i-1][j].start and not self.cells[i-1][j].end:
                    tPreRoad = 1
                    leftShift2 = tPreRoad * 4 % 15
                    if (self.cells[i-1][j].road == (leftShift2 + tPreRoad) % 15) \
                        or (self.cells[i-1][j].road == (leftShift2 + tPreRoad * 2 % 15) % 15) \
                        or (self.cells[i-1][j].road == (leftShift2 + tPreRoad * 8 % 15) % 15):
                        newLstPathType[-1] += 1
                        self.findPath(step+1, newLstPath, newLstPathType, 1, i-1, j)
                        newLstPathType[-1] -= 1
        if preRoad != 8 and j + 1 < self.cols: # 如果向右可走
            #print(f'step:{step}, preRoad:{preRoad}, ({i}, {j})')
            if self.cells[i][j+1].end and self.cells[i][j+1].road == 8:
                newLstPathType[-1] += 2
                self.lstSuccessPaths.append(newLstPath)
                self.lstSuccessPathsType.append(newLstPathType)
                return
            if self.cells[i][j+1].canMove:
                newLstPathType[-1] += 2
                self.findPath(step+1, newLstPath, newLstPathType, 2, i, j+1)
                newLstPathType[-1] -= 2
            else:
                if not self.cells[i][j+1].start and not self.cells[i][j+1].end:
                    tPreRoad = 2
                    leftShift2 = tPreRoad * 4 % 15
                    if (self.cells[i][j+1].road == (leftShift2 + tPreRoad) % 15) \
                        or (self.cells[i][j+1].road == (leftShift2 + tPreRoad * 2 % 15) % 15) \
                        or (self.cells[i][j+1].road == (leftShift2 + tPreRoad * 8 % 15) % 15):
                        newLstPathType[-1] += 2
                        self.findPath(step+1, newLstPath, newLstPathType, 2, i, j+1)
                        newLstPathType[-1] -= 2
        if preRoad != 1 and i + 1 < self.rows: # 如果向下可走
            #print(f'step:{step}, preRoad:{preRoad}, ({i}, {j})')
            if self.cells[i+1][j].end and self.cells[i+1][j].road == 1:
                newLstPathType[-1] += 4
                self.lstSuccessPaths.append(newLstPath)
            ...
            if self.cells[i+1][j].canMove:
                newLstPathType[-1] += 4
                self.findPath(step+1, newLstPath, newLstPathType, 4, i+1, j)
                newLstPathType[-1] -= 4
            else:
                if not self.cells[i+1][j].start and not self.cells[i+1][j].end:
                    tPreRoad = 4
        ...
        if preRoad != 2 and j - 1 >=0: # 如果向左可走
            ...
            if self.cells[i][j-1].canMove:
                newLstPathType[-1] += 8
                self.findPath(step+1, newLstPath, newLstPathType, 8, i, j-1)
                newLstPathType[-1] -= 8
            else:
                if not self.cells[i][j-1].start and not self.cells[i][j-1].end:
                    tPreRoad = 8
