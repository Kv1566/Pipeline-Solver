...
def cells_setting():
    ...
    pwInner = PanedWindow(orient=HORIZONTAL, width=150*cols, height=25)
    for j in range(cols):
        btn = Button(pwInner, text='上', bg='pink')
        #pwInner.add(btn)
        btn.place(width=100, height=25, x=j*140+45, y=0)
    pwCells.add(pwInner)
    for i in range(rows):
        pwInner = PanedWindow(orient=HORIZONTAL, width=150, height=150)
        btn = Button(pwInner, text='左', bg='pink', width=2)
        pwInner.add(btn)
        ...
        btn = Button(pwInner, text='右', width=2, bg='pink')
        pwInner.add(btn)
        pwCells.add(pwInner)
    pwInner = PanedWindow(orient=HORIZONTAL, width=150*cols, height=25)
    for j in range(cols):
        btn = Button(pwInner, text='下', bg='pink')
        #pwInner.add(btn)
        btn.place(width=100, height=25, x=j*140+45, y=0)
    pwCells.add(pwInner)
