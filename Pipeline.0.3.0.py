...
class App(tk.Frame):
    ...
    def initUI(self):
        ...
        self.CAN_MOVE = '#99CC00'
        self.MOVE_NOMOVE = '#00FF00'
        self.ROTATE = '#FFFF00'
        self.MOVE2_CANMOVE = '#660000'
        self.ROTATE_NOMOVE = '#0000FF'
    ...
    def road_bgColor_change(self, p):
        if p.widget.config('bg')[-1] == self.HAVE_ROAD:
            p.widget.config(bg = 'SystemButtonFace')
        else:
            p.widget.config(bg = self.HAVE_ROAD)
    ...
    def frame_bgColor_change(self, p):
        if p.widget.config('bg')[-1] == self.CAN_MOVE:
            p.widget.config(bg = self.MOVE_NOMOVE)
        elif p.widget.config('bg')[-1] == self.MOVE_NOMOVE:
            p.widget.config(bg = self.ROTATE)
        elif p.widget.config('bg')[-1] == self.ROTATE:
            p.widget.config(bg = self.ROTATE_NOMOVE)
        elif p.widget.config('bg')[-1] == self.ROTATE_NOMOVE:
            p.widget.config(bg = self.MOVE2_CANMOVE)
        elif p.widget.config('bg')[-1] == self.MOVE2_CANMOVE:
            p.widget.config(bg = 'SystemButtonFace')
        else:
            p.widget.config(bg = self.CAN_MOVE)
    ...
    def cells_setting(self):
        ...
        for j in range(self.cols):
            self.btn = tk.Button(self.pwInner, text='上', bg='pink')
            #pwInner.add(btn)
        ...
        for i in range(self.rows):
            ...
            self.btn = tk.Button(self.pwInner, text='左', bg='pink', width=2)
            ...
            self.btn = tk.Button(self.pwInner, text='右', width=2, bg='pink')
            self.pwInner.add(self.btn)
            self.pwCells.add(self.pwInner)
        self.pwInner = tk.PanedWindow(orient=tk.HORIZONTAL, width=150*self.cols, height=25)
        for j in range(self.cols):
            self.btn = tk.Button(self.pwInner, text='下', bg='pink')
            #pwInner.add(btn)
            self.btn.place(width=100, height=25, x=j*140+45, y=0)
        self.pwCells.add(self.pwInner)
        #
    ...
    def solve(self):
        for j in range(self.rows):
            for i in range(self.cols):
                if self.btnUp[i][j].config('bg')[-1] == self.HAVE_ROAD:
                    self.cells[i][j].append(True)
                else:
                    self.cells[i][j].append(False)
                #
                if self.btnRight[i][j].config('bg')[-1] == self.HAVE_ROAD:
                    self.cells[i][j].append(True)
                else:
                    self.cells[i][j].append(False)
                #
                if self.btnDown[i][j].config('bg')[-1] == self.HAVE_ROAD:
                    self.cells[i][j].append(True)
                else:
                    self.cells[i][j].append(False)
                #
                if self.btnLeft[i][j].config('bg')[-1] == self.HAVE_ROAD:
                    self.cells[i][j].append(True)
                else:
                    self.cells[i][j].append(False)
                #
                if self.btnCenter[i][j].config('bg')[-1] == self.START_COLOR:
                    ...
                    self.cells[i][j].append(False)
                #
                if self.frmCell[i][j].config('bg')[-1] == self.CAN_MOVE:
                    self.cells[i][j].append(True) # initially movable
                    self.cells[i][j].append(False)
                    self.cells[i][j].append(False)
                    self.cells[i][j].append(False)                
                elif self.frmCell[i][j].config('bg')[-1] == self.MOVE_NOMOVE:
                    self.cells[i][j].append(True) # initially movable
                    self.cells[i][j].append(True) # cannot move after move
                    self.cells[i][j].append(False)
                    self.cells[i][j].append(False)                
                elif self.frmCell[i][j].config('bg')[-1] == self.ROTATE:
                    self.cells[i][j].append(True) # initially movable
                    self.cells[i][j].append(False)
                    self.cells[i][j].append(True) # will rotate when each move
                    self.cells[i][j].append(False)                
                elif self.frmCell[i][j].config('bg')[-1] == self.MOVE2_CANMOVE:
                    self.cells[i][j].append(False)
                    self.cells[i][j].append(False)                
                    self.cells[i][j].append(False)                
