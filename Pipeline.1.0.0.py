...
class App(tk.Frame):
    ...
    def getMovableGroup(self):
        ...
        for i in range(self.rows):
            ...
            for j in range(self.cols):
                ...
                #    continue
                if self.cells[i][j].canMove == True: # CanMove
                    tmpList.append(j)
                else:
                    cells = []
    ...
    def move(self, steps, cells):
        for i in range(len(self.lstGroup)):
            ...
            if len(self.lstMoveSteps) > 0:
                if self.lstGroup[i] == self.lstMoveSteps[-1]:
                    continue
                if self.lstGroup[i][2] == self.lstMoveSteps[-1][2]:
                    continue
            ...
            #              cells[x][y].start, cells[x][y].end, cells[x][y].canMove,
            #             cells[x][y].moveNoMove, cells[x][y].rotate, cells[x][y].move2_canMove)
            cells = self.thisMove(self.lstGroup[i], cells, steps)
            #print(self.times, i)
            #for x in range(self.rows):
            #    for y in range(self.cols):
            #        print(cells[x][y].up, cells[x][y].right, cells[x][y].down, cells[x][y].left, 
            ...
            if self.isSuccess:
                ...
                for i in range(len(self.lstMoveStepsString)):
                    print(self.lstMoveStepsString[i])
                break            
            self.move(steps+1, cells)
