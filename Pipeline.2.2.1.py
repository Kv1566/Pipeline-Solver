...
class cell():
    ...
    def __init__(self, up, right, down, left, start, end, canMove, moveNoMove, rotate, move2_canMove):
        self.up = up
        self.right = right
        self.down = down
        self.left = left
...
class App(tk.Frame):
    ...
    def saveCellsToFile(self):
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                s += str(self.cells[i][j].up) + ',' \
                    + str(self.cells[i][j].right) + ',' \
                    + str(self.cells[i][j].down) + ',' \
                    + str(self.cells[i][j].left) + ',' \
    ...
    def btnMove(self, text, n):
        ...
        if isSuccess:
            (thisI, thisJ) = self.start
            #print('start', thisI, thisJ)
            sFrom = ''
            if self.cells[thisI][thisJ].up:
                sFrom = 'up'
                thisI -= 1
            elif self.cells[thisI][thisJ].right:
                sFrom = 'right'
                thisJ += 1
            elif self.cells[thisI][thisJ].down:
                sFrom = 'down'
                thisI += 1
            elif self.cells[thisI][thisJ].left:
                sFrom = 'left'
                thisJ -= 1
            while(True):
                if self.cells[thisI][thisJ].end == True:
                    break
                self.btnCenter[thisI][thisJ].config(bg = 'red')
                if self.cells[thisI][thisJ].up and (sFrom != 'down'):
                    thisI -= 1
                    sFrom = 'up'
                elif self.cells[thisI][thisJ].right and (sFrom != 'left'):
                    thisJ += 1
                    sFrom = 'right'
                elif self.cells[thisI][thisJ].down and (sFrom != 'up'):
                    thisI += 1
                    sFrom = 'down'
                elif self.cells[thisI][thisJ].left and (sFrom != 'right'):
                    thisJ -= 1
                    sFrom = 'left'
    ...
    def getCells(self):
        ...
        for i in range(self.rows):
            self.cells.append([])
            for j in range(self.cols):
                self.cells[i].append(cell(False, False, False, False, False, False, False, False, False, False))
                #
    ...
    def getMovableGroup(self, cells):
        ...
        for i in range(self.rows):
            ...
            for j in range(self.cols):
                ...
                #if len(cells[i][j]) == 0:
                #    continue
                if cells[i][j].canMove == True: # CanMove
                    tmpList.append(j)
                else:
                    thisCells = []
    ...
    def move(self, steps, cells, lstGroup):
        for i in range(len(lstGroup)):
            ...
            if len(self.lstMoveSteps) > 0:
                if lstGroup[i] == self.lstMoveSteps[-1]:
                    continue
                if lstGroup[i][2] == self.lstMoveSteps[-1][2]:
                    continue
            ...
            #              cells[x][y].start, cells[x][y].end, cells[x][y].canMove,
            #             cells[x][y].moveNoMove, cells[x][y].rotate, cells[x][y].move2_canMove)
            (cells, hasMoveNoMove) = self.thisMove(lstGroup[i], cells, steps)
            if hasMoveNoMove:
    ...
    def check(self, cells):
        ...
        #print('start', thisI, thisJ)
        sFrom = ''
        if cells[thisI][thisJ].up:
            sFrom = 'up'
            thisI -= 1
        elif cells[thisI][thisJ].right:
            sFrom = 'right'
            thisJ += 1
        elif cells[thisI][thisJ].down:
            sFrom = 'down'
            thisI += 1
        elif cells[thisI][thisJ].left:
            sFrom = 'left'
            thisJ -= 1
        while(True):
            #print(thisI, thisJ, cells[thisI][thisJ].end)
            if cells[thisI][thisJ].end == True:
                if ((sFrom == 'up') and cells[thisI][thisJ].down) \
                    or ((sFrom == 'right') and cells[thisI][thisJ].left) \
                    or ((sFrom == 'down') and cells[thisI][thisJ].up) \
                    or ((sFrom == 'left') and cells[thisI][thisJ].right):
                    isFinished = True
                    print('successed')
                    #print('3------')
                    #for x in range(self.rows):
                    ...
                    #             cells[x][y].moveNoMove, cells[x][y].rotate, cells[x][y].move2_canMove)
                    break
            if ((sFrom == 'up') and not cells[thisI][thisJ].down) \
                or ((sFrom == 'right') and not cells[thisI][thisJ].left) \
                or ((sFrom == 'down') and not cells[thisI][thisJ].up) \
                or ((sFrom == 'left') and not cells[thisI][thisJ].right):
                break
            if cells[thisI][thisJ].up and (sFrom != 'down'):
                thisI -= 1
                sFrom = 'up'
                if thisI < 0:
                    break
            elif cells[thisI][thisJ].right and (sFrom != 'left'):
                thisJ += 1
                sFrom = 'right'
                if thisJ > self.cols-1:
                    break
            elif cells[thisI][thisJ].down and (sFrom != 'up'):
                thisI += 1
                sFrom = 'down'
                if thisI > self.rows-1:
                    break
            elif cells[thisI][thisJ].left and (sFrom != 'right'):
                thisJ -= 1
                sFrom = 'left'
                if thisJ < 0:
                    break
