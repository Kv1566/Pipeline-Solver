...
class App(tk.Frame):
    ...
    def check(self, cells):
        ...
        if cells[i][j].up:
            ...
            j -= 1
        while(True):
            #print(i, j, cells[i][j].end)
            if cells[i][j].end == True:
                if ((sFrom == 'up') and cells[i][j].down) \
                    or ((sFrom == 'right') and cells[i][j].left) \
                    or ((sFrom == 'down') and cells[i][j].up) \
                    or ((sFrom == 'left') and cells[i][j].right):
                    isFinished = True
                    print('successed')
                    #print('3------')
                    #for x in range(self.rows):
                    ...
                    #             cells[x][y].moveNoMove, cells[x][y].rotate, cells[x][y].move2_canMove)
                    break
            if ((sFrom == 'up') and not cells[i][j].down) \
                or ((sFrom == 'right') and not cells[i][j].left) \
                or ((sFrom == 'down') and not cells[i][j].up) \
                or ((sFrom == 'left') and not cells[i][j].right):
                break
            if cells[i][j].up and (sFrom != 'down'):
                i -= 1
                sFrom = 'up'
                if i < 0:
                    break
            elif cells[i][j].right and (sFrom != 'left'):
                j += 1
                sFrom = 'right'
                if j > self.cols-1:
                    break
            elif cells[i][j].down and (sFrom != 'up'):
                i += 1
                sFrom = 'down'
                if i > self.rows-1:
                    break
            elif cells[i][j].left and (sFrom != 'right'):
                j -= 1
                sFrom = 'left'
                if j < 0:
                    break
