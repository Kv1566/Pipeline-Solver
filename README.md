...
* ## Pipeline.2.4.0 - 2020/08/16 (目前最新版本—１)： <br>
  ...
  road的數值內容代表如下：
  0 : 代表格子內沒有任何道路
  1 : 代表格子內只有向上(up)的單一通道
  2 : 代表格子內只有向右(right)的單一通道
  3 : 代表格子內有向上及向右(up + right)兩個通道
  4 : 代表格子內只有向下(down)的單一通道
  5 : 代表格子內有向上及向下(up + down)兩個通道
  6 : 代表格子內有向右及向下(right + down)兩個通道
  7 : 略過，遊戲中尚無此種通道
  8 : 代表格子內只有向左(left)的單一通道
  9 : 代表格子內有向左及向上(left + up)兩個通道
  10: 代表格子內有向右及向左(right + left)兩個通道
  11: 略過，遊戲中尚無此種通道
  12: 代表格子內有向下及向左(down + left)兩個通道
  其中，
  1、2、4、8這四種格子為一組，都是單一通道，
  3、6、9、12這四種格子為一組，都是直角型態的雙通道，
